...
- go through help file
- use a non-default stat for a geom
- use after_stat() to use a different variable calculated by a stat (e.g. in geom_histogram())
:::

## Stats

-  Every stat has a default geom and you *usually* use the `geom_*()` function
-  Some stats don't have an equivalent geom, but are still very useful
