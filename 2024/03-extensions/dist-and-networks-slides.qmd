...
penguins_brm <- brm(body_mass_g ~ species, data = penguins)

```

```{r, echo = T}
penguins_brm |>
tidybayes::add_draws(penguins) |>
  head()
```
...
```{r, echo = T}
penguins_brm %>%
  tidybayes::spread_draws(species_mean[species]) %>%
  ggplot(aes(x = species_mean, y = species)) +
